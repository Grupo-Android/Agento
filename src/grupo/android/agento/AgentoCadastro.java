...
public class AgentoCadastro extends Activity {
	...
	public void registrar(View view) {
		if (entradaValida()) {
			...
			// cadastra usuario
			datasource.insertUsuario(usuario.getText().toString(), senha
	...
	private boolean usuarioUnico() {
		...
		for (int i = 0; i < values.size(); ++i)
			if (values.get(i).getUsuario().toString().equals(usuario.getText().toString()))
				return false;
	...
	private boolean entradaValida() {
		boolean usuarioValido = ValidaEntrada.validaUsuario(usuario.getText()
				.toString());
		boolean senhaValida = ValidaEntrada.validaSenha(senha.getText()
				.toString());
		boolean emailValido = ValidaEntrada.validaEmail(email.getText()
				.toString());
		boolean usuarioUnico = usuarioUnico();
		
		mensagemErro = "";
		mensagemErro += (senhaValida) ? ("") : ("-> Senha Inválida\n");
		mensagemErro += (emailValido) ? ("") : ("-> Email Inválido\n");
		mensagemErro += (usuarioValido) ? ("") : ("-> Usuario Inválido\n");
		mensagemErro += (usuarioUnico) ? ("") : ("-> Usuário Existente\n");

		return usuarioValido && senhaValida && emailValido && usuarioUnico;
	}
